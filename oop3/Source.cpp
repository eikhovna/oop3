...
int main()
{
	...
	while (itterations <= 10000)	//цикл для тестирования контейнера
	{
		...
		for (int i = 0; i < itterations; ++i)
		{
			...
			case 2:
				int setget = rand() % 2;
				if (setget == 0) storage.getObject(rand() % itterations)->getsomething(); //геттер объекта(возвращает данные)
				else storage.getObject(rand() % itterations)->setsomething(rand() % 100); //сеттер(устанавливает данные)
				break;
			}
		}
		clock_t end = clock();
